 ...
 * ============================================================ */
function buildSystemPrompt(skills, workspaceRoot, mcpTools = []) {
...
function startSpinner(label) {
    ...
    spinnerTimer = setInterval(() => {
        ...
        process.stdout.write(`\r\x1b[0K  ${colors.cyan}${frame}${colors.reset} ${currentSpinnerLabel}`);
...
function displayEvent(event) {
    switch (event.type) {
        ...
        case "tool_execution_update": {
            const update = event.partialResult;
            if (update?.type === "stdout") {
                ...
                if (bashLastLines.length > 6)
                    bashLastLines = bashLastLines.slice(-6);
        ...
        case "tool_execution_end": {
            ...
            if (event.toolName === "bash") {
                ...
                writeln(`${colors.dim}  ╰─${colors.reset}${colors.green} done${colors.reset}${colors.dim}${sizeInfo}${errInfo}${colors.reset}`);
            }
            break;
        }
        case "turn_end": {
...
function isInsideWorkspace(targetPath, workspace) {
...
function isWriteOrRiskyCommand(command) {
...
async function checkAllCommandSegments(command, workspaceRoot) {
...
async function confirmAction(rl, message) {
    ...
    const answer = await rl.question(`  ${colors.dim}[Enter]${colors.reset} confirm  ${colors.dim}[n]${colors.reset} reject > `);
    writeln("");
...
function createAgent(model, systemPrompt, thinkingLevel = "low", workspaceRoot, rl, mcpTools = []) {
    ...
    return new Agent({
        ...
        beforeToolCall: !workspaceRoot ? undefined : async (params) => {
            ...
            if (toolName === "bash") {
                ...
                if (dangerous && rl) {
                    const ok = await confirmAction(rl, `Dangerous command detected: ${dangerous}\n  Command: ${command.slice(0, 100)}`);
                    if (!ok) {
                ...
                if (cwd && !isInsideWorkspace(cwd, workspaceRoot) && rl) {
                    const ok = await confirmAction(rl, `bash cwd outside workspace: ${cwd}`);
                    if (!ok) {
                ...
                if (externalPath && rl) {
                    const ok = await confirmAction(rl, `Write/risky command targets path outside workspace: ${externalPath}\n  Command: ${command.slice(0, 80)}`);
                    if (!ok) {
                        return {
                            block: true,
            ...
            if (toolName === "write" || toolName === "edit") {
                ...
                if (!isInsideWorkspace(filePath, workspaceRoot) && rl) {
                    const ok = await confirmAction(rl, `${toolName} target outside workspace: ${filePath}`);
                    if (!ok) {
