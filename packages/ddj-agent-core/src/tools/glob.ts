...
export const globTool: AgentTool = {
  ...
  parameters: Type.Object({
    ...
    path: Type.Optional(
      Type.String({ description: "Directory to search in (defaults to current working directory)" })
    ),
  }),
  async execute({ args }) {
    ...
    try {
      ...
      return {
        content: [{ type: "text", text: `Error searching with glob "${pattern}": ${(err as Error).message}` }],
      };
    }
  },
};
