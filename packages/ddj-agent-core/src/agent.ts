...
export class Agent {
  ...
  private _eventStream: EventStream<AgentEvent, AgentMessage[]>;
  ...
  constructor(config: AgentConfig) {
    ...
    this._eventStream = new EventStream<AgentEvent, AgentMessage[]>(
  ...
  async waitForIdle(): Promise<AgentMessage[]> {
  ...
  private async _run(): Promise<AgentMessage[]> {
  ...
  private async _agentLoop(): Promise<void> {
    ...
    while (true) {
      ...
      if (this._config.transformContext) {
        contextMessages = await this._config.transformContext(
          contextMessages,
          this._signal?.signal
        );
      }
      ...
      if (toolCalls.length > 0) {
        ...
        const isSequential =
          this._config.toolExecution === "sequential" || hasSequentialTool;
        ...
        if (isSequential) {
          ...
          const results = await Promise.all(
            toolCalls.map((tc) => this._executeTool(tc))
          );
        ...
        const orderedResults = toolCalls.map((tc) => {
          return (
            resultMap.get(tc.id) || {
              ...
              toolCallId: tc.id,
              toolName: tc.name,
              content: [{ type: "text" as const, text: "Tool result not found" }],
              isError: true,
              timestamp: Date.now(),
            }
          );
  ...
  private async _callLlm(
    ...
    const partialMsg: AgentAssistantMessage = {
      role: "assistant",
      content: [],
      stopReason: "unknown",
    };
  ...
  private async _executeTool(
    ...
    if (this._config.beforeToolCall) {
      ...
      if (hookResult?.block) {
        this._state.pendingToolCalls = new Set(
          [...this._state.pendingToolCalls].filter((id) => id !== toolCall.id)
        );
        this._emit({
