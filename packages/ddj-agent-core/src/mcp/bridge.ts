...
import type { ContentBlock } from "@ddj-ai/core";

/** Convert a JSON Schema-like object to TypeBox-like schema (minimal) */
function jsonSchemaToTypeBoxParameters(schema: Record<string, unknown>): Record<string, unknown> {
...
export function mcpToAgentTool(
  ...
  return {
    ...
    parameters: jsonSchemaToTypeBoxParameters(
      mcpTool.inputSchema || { type: "object", properties: {} }
    ) as import("typebox").TSchema,
    async execute({ args }) {
      try {
        ...
        return {
          content: [{
            type: "text",
            text: `MCP tool error (${client.serverName}/${mcpTool.name}): ${(err as Error).message}`,
          }],
...
export function mcpServerToAgentTools(
  client: MCPClient
): AgentTool[] {
